	...
	public function display_cart() {
		...
		if($this->itemCount > 0) {
			...
			foreach($this->get_contents() as $item)	{
                ...
                echo tab(7) . "<div class='form-group'>";
				echo tab(8) . "<input id='jcartItemQty-{$item['id']}' name='jcartItemQty[]' size='2' type='text' value='{$item['qty']}' />\n";
                echo tab(7) . "</div>";
				echo tab(6) . "</td>\n";
				echo tab(6) . "<td class='jcart-item-name'>\n";

				if ($item['url']) {
					echo tab(7) . "<h4><span class='label label-primary '><a class='nameLable' href='{$item['url']}'>{$item['name']}</a></span></h4>\n";
				}
				else {
				...
				echo tab(7) . "<h5><span class='label label-danger '><a class=' nameLable jcart-remove' href='?jcartRemove={$item['id']}'>{$config['text']['removeLink']}</a></span></h5>\n";
				echo tab(6) . "</td>\n";
				echo tab(5) . "</tr>\n";
			}
		}else {// The cart is empty
			echo tab(5) . "<tr><td id='jcart-empty'>{$config['text']['emptyMessage']}</td></tr>\n";
		}
